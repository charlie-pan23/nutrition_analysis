<template>
  <view class="mescroll-uni-warp">
    <scroll-view :id="viewId" :class="{'mescroll-uni-fixed':isFixed}" :enable-back-to-top="true" :scroll-top="scrollTop" :scroll-with-animation="scrollAnim" :scroll-y='scrollable' :style="{'height':scrollHeight,'padding-top':padTop,'padding-bottom':padBottom,'top':fixedTop,'bottom':fixedBottom}" :throttle="false" class="mescroll-uni" @scroll="scroll">
      <view :change:prop="wxsBiz.propObserver"
            :prop="wxsProp"
        ...
        <!-- 状态栏 -->
        <view v-if="topbar&&statusBarHeight" :style="{height: statusBarHeight+'px', background: topbar}" class="mescroll-topbar"></view>

        <view :change:prop="wxsBiz.callObserver" :prop="callProp" :style="{'transform': translateY, 'transition': transition}" class="mescroll-wxs-content">
          <!-- 下拉加载区域 (支付宝小程序子组件传参给子子组件仍报单项数据流的异常,暂时不通过mescroll-down组件实现)-->
          <!-- <mescroll-down :option="mescroll.optDown" :type="downLoadType" :rate="downRate"></mescroll-down> -->
          <view v-if="mescroll.optDown.use" :style="{'background':mescroll.optDown.bgColor,'color':mescroll.optDown.textColor}" class="mescroll-downwarp">
            <view class="downwarp-content">
              <view :class="{'mescroll-rotate': isDownLoading}" :style="{'border-color':mescroll.optDown.textColor, 'transform': downRotate}" class="downwarp-progress mescroll-wxs-progress"></view>
          ...
          <view v-if="mescroll.optUp.use && !isDownLoading && upLoadType!==3" :style="{'background':mescroll.optUp.bgColor,'color':mescroll.optUp.textColor}" class="mescroll-upwarp">
            <!-- 加载中 (此处不能用v-if,否则android小程序快速上拉可能会不断触发上拉回调) -->
            <view v-show="upLoadType===1">
              <view :style="{'border-color':mescroll.optUp.textColor}" class="upwarp-progress mescroll-rotate"></view>
        ...
        <!-- #ifdef H5 -->
        <view v-if="bottombar && windowBottom>0" :style="{height: windowBottom+'px'}" class="mescroll-bottombar"></view>
        <!-- #endif -->
...
<!-- #ifdef MP-WEIXIN || MP-QQ || APP-PLUS || H5 -->
<script lang="wxs" module="wxsBiz" src="./wxs/wxs.wxs"></script>
<!-- #endif -->
...
export default {
  ...
  methods: {
    ...
    toPx (num) {
      ...
      return num ? num.indexOf('calc') !== -1 ? num : uni.upx2px(Number(num)) : 0
    },
