...
def run():
    user = "difyuser"
    wav_file_path = "recording.wav"  # 替换为你实际的 WAV 文件路径
    ...
    print(file_id)
    file_id2 = upload_wav_file(wav_file_path, user)
...
def convert_wav_with_ffmpeg(input_path, output_path):
    try:
        # 使用 ffmpeg 强制转换文件
        command = ['ffmpeg', '-i', input_path, '-acodec', 'pcm_s16le', '-ar', '44100', '-ac', '2', output_path]
...
# 文件路径
input_wav_path = saved_path  # 原始文件路径
